...
Sgml-stream offers five useful built-in base classes. For a historical note, [click here](#user-elements-in-version-zero). These should suffice for most uses. There are, however, other useful concepts out there that are not touched on by these five. If you want to use such a concept, you can write your own base class. The built-in base classes are:
